...
from src.duffel_client.endpoints.flights import (
    ...
    create_flight_booking,   # you already have this
    fetch_flight_offer,      # optional: to confirm base price/currency
)
...
def normalize_seat_maps(seat_maps_raw) -> tuple[list, int]:
    ...
    Input shape (from src.duffel_client.endpoints.flights.get_seat_maps):
      {
        "raw_data": {...},
        "formatted_seats": [
          {
    ...
    Returns:
      (available_seats, total_available) where each seat:
      {
        "service_id": "...",
        "label": "28C",
        "cabin": "economy",
        "position": "window" | "aisle" | "middle" | None,
        "price": "40.30",
        "currency": "AUD",
      }
    """
    ...
    try:
        ...
        if name == "search_hotels_ui":
            if not search_hotels_tool:
                return types.ServerResult(types.CallToolResult(
                    content=[types.TextContent(type="text", text="search_hotels_tool is not available on this server.")],
                    isError=True,
                ))
            ...
            return types.ServerResult(types.CallToolResult(
                content=[res, types.TextContent(type="text", text=f"Found {normalized['meta'].get('count', 0)} hotels.")],
                structuredContent=normalized,
                _meta=meta,
            ))
        ...
        if name == "fetch_hotel_rates_ui":
            if not fetch_hotel_rates_tool:
                return types.ServerResult(types.CallToolResult(
                    content=[types.TextContent(type="text", text="fetch_hotel_rates_tool is not available on this server.")],
                    isError=True,
                ))
            ...
            if not srr:
                return types.ServerResult(types.CallToolResult(
                    content=[types.TextContent(type="text", text="search_result_id (srr_...) is required")],
                    isError=True,
                ))
            if not str(srr).startswith("srr_"):
                return types.ServerResult(types.CallToolResult(
                    content=[types.TextContent(type="text", text=f"Invalid search_result_id: {srr}")],
                    isError=True,
                ))
            ...
            return types.ServerResult(types.CallToolResult(
                content=[res, types.TextContent(type="text", text=f"Found {normalized['meta']['count']} room options.")],
                structuredContent=normalized,
                _meta=meta,
            ))
       ...
       # --- Start hotel payment WITHOUT graph: just build Stripe Checkout and show link in chat ---
        ...
        if name == "finalize_hotel_checkout":
            ...
            # If you already have helpers like create_quote/create_booking, call them here.
            # Otherwise, we return a minimal success payload as a stub (so the UI flow works).
            try:
                # Example (uncomment and implement in your project):
                #
                # quote = await create_quote(rate_id)  # returns {"data":{"id": "...", ...}}
                # qid = quote["data"]["id"]
                ...
                #     email=email,
                #     phone_number=phone,
                #     stay_special_requests=stay_reqs,
                #     payment={"stripe_payment_intent_id": pi_id},
                # )
                #
                # For now: stub a minimal booking response you can replace later.
                booking = {
...
async def success_route(request: Request):
    ...
    try:
        session = stripe.checkout.Session.retrieve(
            session_id, expand=["payment_intent", "customer_details"]
        )
        if (session.get("payment_status") or "").lower() != "paid":
        ...
        return PlainTextResponse(f"✅ Booking confirmed • Ref: {ref}\nPaid {ses_ccy} {paid:.2f}", status_code=200)
    except Exception as e:
...
async def flight_success_route(request: Request):
    ...
    try:
        ...
        return PlainTextResponse(f"✈️ Booking confirmed • Ref: {ref}\nPaid {ses_ccy} {paid_display:.2f}", status_code=200)
    except Exception as e:
